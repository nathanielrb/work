...
**** TODO Finalize vocabulary for storing in triple-store
     Finalize vocabulary. 
...
*** Subscribe to updates from the Delta service, in order to update the indexes :v1:
*** Invalidate the index when destructive updates arrive                 :v1:
*** Automatically recalculate the index, when an invalidated index exists :v1:
...
#+CAPTION: Clock summary at [2018-11-12 lun. 19:09]
| Headline                      |    Time |      |
|-------------------------------+---------+------|
| *Total time*                  | *10:00* |      |
|-------------------------------+---------+------|
...
| \emsp Access Rights           |         | 1:45 |
| \emsp Querying multiple types |         | 7:00 |
| Admin                         |    1:00 |      |
