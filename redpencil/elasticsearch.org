...
*** Handle simultaneous index updating with proper threading             :v1:
...
*** DONE Subscribe to deltas                                             
    Subscribe to updates from the Delta service, in order to update
    the indexes
*** DONE Invalidate the index when destructive updates arrive            
*** DONE Automatically recalculate the index                             
    When an invalidated index exists
*** TODO Use 'graph' from deltas to check for correct graph
